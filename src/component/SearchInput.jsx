...
const SearchInput = () => {
  ...
  return (<div className="flex items-center justify-between p-5 border-b-[1.5px] border-second dark:border-darkOuter border-solid md:px-8 md:py-4">
    <img src={searchIcon} alt="something's wrong" className="w-9"/>
    <input type="text" placeholder="find city" onKeyUp={search} ref={input} value={searchInput} onInput={event => setSearchInput(event.target.value)} className="w-0 flex-1 bg-transparent text-lg placeholder:text-second dark:placeholder:text-darkOuter placeholder:capitalize placeholder:font-medium placeholder:text-lg outline-none px-3 sm:text-xl sm:placeholder:text-xl sm:px-7 md:text-lg md:placeholder:text-lg md:px-3"/>
